...
{
    DO_Log(log4cpp::Priority::ERROR, msg);
}
void CRLLog::Log_DEBUG(const char*  msg)
{
    DO_Log(log4cpp::Priority::DEBUG, msg);
}
void CRLLog::Log_NOTICE(const char*  msg)
{
    DO_Log(log4cpp::Priority::NOTICE, msg);
}


void CRLLog::DO_Log(log4cpp::Priority::PriorityLevel lv,const char*  msg)
{
    if (!_isInited)
        return;
    if(_sUid.empty())
