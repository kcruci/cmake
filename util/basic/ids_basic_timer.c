#include "ids_basic_timer.h"
#include "ids_basic_shm.h"

#include "ids_basic_list.h"
...
#define ERROR_LOG(fmt, args...) do { \
	if((NULL == pstLog) || (0 >= stTimerHash.iLogLevel)) break; \
		Log(pstLog, 2, "%s:%d(%s)[ERROR]: "fmt, 	__FILE__, __LINE__, __FUNCTION__ , ## args); \
} while (0)

#define DEBUG_LOG(fmt, args...) do { \
	if((NULL == stTimerHash.pstLog) || (2 >= stTimerHash.iLogLevel)) break; \
		Log(pstLog, 2, "%s:%d(%s)[DEBUG]: "fmt, 	__FILE__, __LINE__, __FUNCTION__ , ## args); \
} while (0)
		...
		unsigned uiCheckIntervalUs, unsigned uiType, int isessionKey)
{
	...
	}
	{
		...
		printf("hash table size %u, node size %u * node Count %u\n", uTableSize, sizeof(TimerNode)+uMaxUserDataSize, uTableSize / (sizeof(TimerNode)+uMaxUserDataSize));
#if 0
		if (GetShm2(&pTable, isessionKey, uTableSize, 0660|IPC_CREAT) < 0) 
		{
		...
		{
			DEBUG_LOG("get shm mem OK! all:%d %d", uRowNum, auNodeNum[0]);
		}

#else
		pTable = malloc(uTableSize);
		if(NULL == pTable) {
			ERROR_LOG("malloc mem failed! No enough memory.");
			return -10;
		}
#endif	
		memset(pTable, 0, uTableSize);
		iRet = HashTableInit(&stTimerHash, pstLog, iLogLevel, pTable, uTableSize, 
				sizeof(TimerNode) + uMaxUserDataSize, uRowNum, auNodeNum, auNodeNum, CmpTimerHashNode);
		if(0 > iRet) {
			ERROR_LOG("HashTableInit error return: %d", iRet);
			return iRet;
		}
	}
	{
		...
		if(0 >= uiCheckInvlUs) {
			ERROR_LOG("BUG: uiCheckInvlUs can NOT set to be zero!");
			return -2;
		}
...
/*
 * use these logs when need
 */
#undef ERROR_LOG
/* LOG_LEVEL 1 */
#define ERROR_LOG(fmt, args...) do { \
	if((NULL == stTimerHash.pstLog) || (0 >= stTimerHash.iLogLevel)) break; \
		Log(stTimerHash.pstLog, 2, "%s:%d(%s)[ERROR]: "fmt, 	__FILE__, __LINE__, __FUNCTION__ , ## args); \
} while (0)
#undef WARN_LOG
/* LOG_LEVEL 2 */
#define WARN_LOG(fmt, args...) do { \
	if((NULL == stTimerHash.pstLog) || (1 >= stTimerHash.iLogLevel)) break; \
		Log(stTimerHash.pstLog, 2, "%s:%d(%s)[WARNNING]: "fmt, 	__FILE__, __LINE__, __FUNCTION__ , ## args); \
} while (0)
#undef DEBUG_LOG
/* LOG_LEVEL 3 */
#define DEBUG_LOG(fmt, args...) do { \
	if((NULL == stTimerHash.pstLog) || (2 >= stTimerHash.iLogLevel)) break; \
		Log(stTimerHash.pstLog, 2, "%s:%d(%s)[DEBUG]: "fmt, 	__FILE__, __LINE__, __FUNCTION__ , ## args); \
} while (0)
#undef INFO_LOG
/* LOG_LEVEL 4 */
#define INFO_LOG(fmt, args...) do { \
	if((NULL == stTimerHash.pstLog) || (3 >= stTimerHash.iLogLevel)) break; \
		Log(stTimerHash.pstLog, 2, "%s:%d(%s)[INFO]: "fmt, 	__FILE__, __LINE__, __FUNCTION__ , ## args); \
} while (0)
#undef TRACE_LOG
/* LOG_LEVEL 5 */
#define TRACE_LOG(fmt, args...) do { \
	if((NULL == stTimerHash.pstLog) || (4 >= stTimerHash.iLogLevel)) break; \
		Log(stTimerHash.pstLog, 2, "%s:%d(%s)[TRACE]: "fmt, 	__FILE__, __LINE__, __FUNCTION__ , ## args); \
} while (0)
		...
		ExpireFunc OnExpire, unsigned uiDataLen, char * sData)
{
	...
	if(uiTimerType != NON_CONST_DATA_LEN_TIMER)
	{
		if(uiDataLen + sizeof(TimerNode) > stTimerHash.uNodeSize) {
			ERROR_LOG("BUG: datalen [%u] is too long, must not longer than [%u]", 
	...
	while(iExist) 
	{
		...
		if(uiLastSeq == uiSeq) 
		{
			ERROR_LOG("Error: too many timers, reaching the timer limit!");
			return -1;
		}
		stNode.uiSeq = uiSeq;
		if(uiTimerType != NON_CONST_DATA_LEN_TIMER) 
		{
							...
							uiSeq, &iExist))) 
			{
				//Need Warnning here?
				//Attr_API(, 1);
				WARN_LOG("Error: too many timers. the timer hash is almost used up!");
				return -2;
			}
		} 
		else
	...
	pstNode->uiSeq = uiSeq;
	gettimeofday(&stTV, NULL);
	/*Begin modify by Frank.cheng at 2015.03.03 */
	//pstNode->uiExpire = (unsigned)(USDIFF(stTV, stBaseTV) / uiCheckInvlUs + uiTimeOut);
	pstNode->uiExpire = (unsigned)((USDIFF(stTV, stBaseTV)  + uiTimeOut)/ uiCheckInvlUs);
	/*End modify by Frank.cheng at 2015.03.03 */
		...
		if (pstCurr != pstHead) {
			...
			if(uiTimerType != NON_CONST_DATA_LEN_TIMER) {
				...
				if(0 > iRet) {
					uiDataLen = 0;
					OnExpire = NULL;
					TRACE_LOG("GetClrTimerHashData Error:%d", iRet);
				}
			}
