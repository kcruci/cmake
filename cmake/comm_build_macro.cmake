...
#   )
#
# 4) custom macro to build *c *cpp => ../lib/lib*.so
...
#   )
#
# 5) custom macro to build *.proto => *.h *.cc
# BUILD_PB2HCC(targetname src1 src2 ...
#   [OUTPUT_DIR output_dir]             # default ./
#   )
#
# 6) custom macro to build *cpp => ../bin/*.exe
# BUILD_EXE_LIST(targetname src1 src2 ...
#   [EXE_TYPE typename]             # default "FCGI"
#   [OUTPUT_DIR output_dir]             # default ../bin/
#   [DEFINES define1 define2 ...]
#   [INCLUDE_DIRS include_dir1 include_dir2 ...]
#   [LIB_DIRS lib_dir1 lib_dir2 ...]
...
#   )
#
# 7) custom macro to build *unittest.cpp => *unitetest.exe
...
#
#
# 8) TODO custom macro to build *.proto => *.json.template
# .proto => conf json template
#
#########################################################################
...
# custom macro to build *cpp => ../bin/*.exe
# BUILD_EXE_LIST(targetname src1 src2 ...
#   [EXE_TYPE typename]             # default "FCGI"
#   [OUTPUT_DIR output_dir]             # default ../bin/
#   [DEFINES define1 define2 ...]
...
macro(BUILD_EXE_LIST)
    ...
    car(BUILD_TARGET ${BUILD_UNPARSED_ARGUMENTS})
    cdr(BUILD_FCGI_SRC_LIST ${BUILD_UNPARSED_ARGUMENTS})

    if(NOT BUILD_FCGI_SRC_LIST)
        message(FATAL_ERROR "\${SRC_LIST} NOTFOUND")
    elseif(CUSTOM_MACRO_VERBOSE)
        message(STATUS "\${SRC_LIST}: ${BUILD_FCGI_SRC_LIST}")
    endif()
    if(NOT BUILD_OUTPUT_DIR)
        set(BUILD_OUTPUT_DIR ${CMAKE_CURRENT_SOURCE_DIR}/../bin)
    endif()
    ...
    foreach(fcgicpp ${BUILD_FCGI_SRC_LIST})
        if(NOT EXISTS ${fcgicpp})
            message(FATAL_ERROR "\${CGI_SRC}: ${fcgicpp} NOT EXISTS")
        endif()
        get_filename_component(cginame ${fcgicpp} NAME_WE)
        message(STATUS "BUILD_${BUILD_TARGET}(${cginame})")
        add_executable(${cginame} ${fcgicpp})
        set_target_properties(${cginame} PROPERTIES RUNTIME_OUTPUT_NAME  "${cginame}")
        set_target_properties(${cginame} PROPERTIES RUNTIME_OUTPUT_DIRECTORY "${BUILD_OUTPUT_DIR}")
        if(BUILD_DEFINES)
            if(CUSTOM_MACRO_VERBOSE)
                message(STATUS "\${DEFINES}: ${BUILD_DEFINES}")
            endif()
            target_compile_definitions(${cginame} PUBLIC ${BUILD_DEFINES})
        endif()
        if(BUILD_INCLUDE_DIRS)
            if(CUSTOM_MACRO_VERBOSE)
                message(STATUS "\${INCLUDE_DIRS}: ${BUILD_INCLUDE_DIRS}")
            endif()
            target_include_directories(${cginame} PUBLIC ${BUILD_INCLUDE_DIRS})
        endif()
        if(BUILD_LIBS)
            if(CUSTOM_MACRO_VERBOSE)
                message(STATUS "\${LIBS}: ${BUILD_LIBS}")
            endif()
            target_link_libraries(${cginame} ${BUILD_LIBS})
        endif()
        if(BUILD_DEPENDS)
            if(CUSTOM_MACRO_VERBOSE)
                message(STATUS "\${DEPENDS}: ${BUILD_DEPENDS}")
            endif()
            add_dependencies(${cginame} ${BUILD_DEPENDS})
        endif()
